	...
	class BARCODE
	{
		...
		*/
		...
		function _c93Barcode($barnumber,$scale=1,$file="",$checkdigit=false)
		{
			...
			if ($scale<1) $scale=2;
			$total_y=(double)$scale * $this->_height+$this->_fontscale*$scale;
			if (!$space)
			...
			$xpos=$scale*strlen($bars)+2*$scale*$this->_fontscale; 

			/* allocate the image */
			$total_x= $xpos +$space['left']+$space['right'];
			$xpos=$space['left']+$scale*$this->_fontscale;

		    $height=floor($total_y-($scale*($this->_fontscale*2)));
			...
			$font_arr=@imagettfbbox ( $scale*$this->_fontscale, 0, $this->_font, $barnumber);
			$x= floor($total_x-(int)$font_arr[0]-(int)$font_arr[2]+$scale*$this->_fontscale)/2;
			@imagettftext($im,$scale*$this->_fontscale,0,$x, $height2, $bar_color,$this->_font , $barnumber);
		...
		*/
		...
		function _upceBarcode($barnumber,$scale=1,$file="")
		{
			...
			@imagettftext($im,$scale*$this->_fontscale,0, $space['left'], $height, $bar_color,$this->_font , $encbit);

			$x= $space['left']+$scale*strlen($barnumber)+$scale*6;
			@imagettftext($im,$scale*$this->_fontscale,0,$x, $height2, $bar_color,$this->_font , $barnumber);

			$x=$total_x-$space['left']-$scale*6;
			@imagettftext($im,$scale*$this->_fontscale,0, $x, $height, $bar_color,$this->_font , $checkdigit);
		...
		*/
		...
		function _ean8Barcode($barnumber,$scale=1,$file="")
		{
			...
			@imagettftext($im,$scale*$this->_fontscale,0,$x, $height2, $bar_color,$this->_font , $str);
		...
		*/
		...
		function _eanBarcode($barnumber,$scale=1,$file="")
		{
			...
			@imagettftext($im,$scale*$this->_fontscale,0, $space['left'], $height, $bar_color,$this->_font , $str);
			...
			if($this->_encode=="UPC-A")
			{
				...
				$x=$total_x-$space['left']-$scale*6;
				@imagettftext($im,$scale*$this->_fontscale,0, $x, $height, $bar_color,$this->_font , $str);
			}
