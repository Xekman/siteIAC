...
{
	//file_put_contents($TASK_FILE_ROOT_PATH."cron/1.txt", $row_branch['BID']." ".$row_branch['BSupportEmail']. chr(13) . chr(10), FILE_APPEND | LOCK_EX);
	$task_result = false;
	$bid = $row_branch['BID'];;
	...
	{
		//file_put_contents($TASK_FILE_ROOT_PATH."cron/1.txt", $emailuser. chr(13) . chr(10), FILE_APPEND | LOCK_EX);
		$task_result = false;
		goto gt_exit;
	} 
			...
			if (is_numeric($hot_line_number))
			{
				...
				} else
				{
					...
					if ($request_number != "")
					{
						...
						');
						//file_put_contents($TASK_FILE_ROOT_PATH."cron/1.txt", $request_number. chr(13) . chr(10), FILE_APPEND | LOCK_EX);
					}
					if ($request_number == "")
					{
